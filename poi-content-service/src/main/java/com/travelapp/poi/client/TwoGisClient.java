...
public class TwoGisClient {
    ...
    private List<TwoGisRawPoiDto> parseResponse(JsonNode response, String requestedType) {
        ...
        for (JsonNode item : items) {
            ...
            if (!passesStrictTypeFilter(item, requestedType)) {
                continue;
            }
            ...
            String description = buildDescription(item, name, purposeName, rubricNames, address, features);
    ...
    private boolean hasFeature(Map<String, String> features, String... keywords) {
        for (Map.Entry<String, String> entry : features.entrySet()) {
            ...
            if (!"true".equalsIgnoreCase(value) && StringUtils.isBlank(value)) {
                continue;
            }
    ...
    private boolean passesStrictTypeFilter(JsonNode item, String requestedType) {
        ...
        return switch (requestedType.toLowerCase(Locale.ROOT)) {
            ...
            case "cafe" -> containsAny(text,
                    "кафе", "кофейня", "coffee");
            case "restaurant" -> containsAny(text,
                    "ресторан", "бар", "паб", "пиццерия", "бургер", "столовая");
            case "hotel" -> containsAny(text,
